...
String prepareSendString() {
   int led1 = digitalRead(LED1);
    ...
    return "led1=" + String(led1) + "&led2=" + String(led2) + "&led3=" + String(led3) + "&led4=" + String(led4) + "&led5=" + String(led5) + "&led6=" + String(led6);
}
void sendLEDStatus() {
  if (notConnected || !hasInternetAccess) return;
  onSend = true;

String httpData = prepareSendString();
  ...
  for (;;) {
    ...
    if (!digitalRead(portalTrigerPin)) {
      ...
      if (!digitalRead(portalTrigerPin)) {
        ...
        if (!wifiManager.startConfigPortal("SMARTIZER-CONFIG-WiFi", "123456789")) {
...
void setup() {
 ...
 wifiManager.setConfigPortalTimeout(1);
  if (!wifiManager.autoConnect("remState", "987612345")) {
...
void loop() {
  ...
  if (status == "active" && !taskResumed) vTaskResume(nearbyTask);
}
